...
public class Patient implements Comparable<Patient>{
    ...
    public int compareTo(Patient other){
        /*# YOUR CODE HERE */
        int value = 0; // Initialize the result of comparison
        // Compare priority values
        if(this.priority > other.getPriority()){
            value = 1; // This object has higher priority
        }
        else if(this.priority < other.getPriority()){
            value = -1;   // This object has lower priority
        }
        else if(this.priority == other.getPriority()){  // If priorities are the same
            // Compare total wait times
            if(this.totalWaitTime > other.getTotalWaitingTime()){
                value = 1;  // This object has longer total wait time
            }
            else if(this.totalWaitTime < other.getTotalWaitingTime()){
                value = -1;   // This object has shorter total wait time
            }
        }
        return value;   // Return the comparison result
    }
